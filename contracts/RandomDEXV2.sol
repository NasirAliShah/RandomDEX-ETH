...
/**
 * @title RandomDEXV2
 * @notice RandomDEXV2 is a token with automatic ETH fee conversion.
 */
contract RandomDEXV2 is ERC20, ERC20Permit, AccessControl, ERC20Fee {
    ...
    constructor(
        ...
        ERC20("RandomDEXV2", "RDXV2")
        ERC20Permit("RandomDEXV2")
        ERC20Fee(
            defaultAdmin_,
            feeCollector_,
    ...
    function _update(address from, address to, uint256 amount) internal virtual override(ERC20) {
        ...
        if (fee > 0) {
            ...
            emit Debug("Eth amount after swapping function", ethAmount);
    ...
    function _swapRDXForETH(uint256 rdxAmount) internal returns (uint256 ethAmount) {
        ...
        emit Debug("Inital eth balance", initialETHBalance);
        
        emit Debug("Contract RDX Balance Before Swap", balanceOf(address(this)));
        ...
        emit Debug("Swapped eth value from rdx", ethAmount);
