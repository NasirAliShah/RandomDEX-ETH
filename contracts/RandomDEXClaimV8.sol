    ...
    RandomDEXErrors
{
    ...
    constructor(
        ...
        ERC20("RandomDEXClaimV8", "RDXCV8")
        ERC20Permit("RandomDEXClaimV8")
        ERC20Fee(
            defaultAdmin_,
            feeCollector_,
    ...
    /**
     * @dev Checks if transfers are currently supervised.
     * @return True if transfers are supervised, false otherwise.
     */
    function isTransferSupervised() public view returns (bool) {
        return listingTimestamp > 0 && block.timestamp < listingTimestamp;
    }
    /**
    ...
    function _update(address from, address to, uint256 amount) internal virtual override {
        ...
        // Check if we're in the supervised transfer period
        if (isTransferSupervised()) {
            // During supervised period, check if transaction involves admin or bridge role
            bool isAdminInvolved = hasRole(DEFAULT_ADMIN_ROLE, from) || 
                                  hasRole(DEFAULT_ADMIN_ROLE, to) || 
                                  hasRole(DEFAULT_ADMIN_ROLE, _msgSender());
                                  
            bool isBridgeAuthorized = hasRole(ALLOWED_TRANSFER_FROM_ROLE, _msgSender()) || hasRole(ALLOWED_TRANSFER_FROM_ROLE, to);
            
            if (!isAdminInvolved && !isBridgeAuthorized) {
