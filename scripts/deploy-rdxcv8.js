...
async function main() {
  ...
  console.log("Deploying RandomDEXClaimV8 with account:", deployer.address);
  ...
  const listingTimestamp = Math.floor(Date.now() / 1000) + 2400; // 40 minutes from now

  // Explicitly specify the contract from RandomDEXV6.sol
  const RandomDEX = await ethers.getContractFactory("contracts/RandomDEXClaimV8.sol:RandomDEXClaimV8");
  ...
  console.log("RandomDEXClaimV8 deployed to:", randomDEXAddress);
